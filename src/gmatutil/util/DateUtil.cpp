...
// ajfg
/**
 * Determines if input date string is valid Julian Date or not.
 *   Valid format is nnnnn.ssssss
 *   For example, 21544.99962962963
 *
 * @param  greg  input UTC or TAI Julian Date string
 * @param  checkDate check for valid date (i.e. occurs after Sputnik launch)
 *
 * @return true if time is in valid Julian format; otherwise, false
 */
bool DateUtil::IsValidJulian(const std::string &str, bool checkDate)
{
    if (str.empty() == true)
        return false;
    ...
    MessageInterface::ShowMessage
        ("DateUtil::IsValidJulian() parts=%s, %s\n", parts[0].c_str(),
        parts[1].c_str());
    #endif

    Integer part1, part2;

    if (!GmatStringUtil::ToInteger(parts[1], part1))
        return false;

    if (!GmatStringUtil::ToInteger(parts[2], part2))
        return false;
 ...
 */
bool DateUtil::IsValidGregorianExtra(const std::string &str, bool checkDate, Integer format)
{
    if (str.empty() == true)
        return false;

    if (format == 1) {
        return DateUtil::IsValidGregorian(str, checkDate);
    } else if (format == 2) {
        StringArray parts = GmatStringUtil::SeparateBy(str, "T");
        if (parts.size() != 2)
            return false;

        #if DEBUG_DATE_VALIDATE
        MessageInterface::ShowMessage
            ("DateUtil::IsValidGregorianExtra() parts=%s, %s\n", parts[0].c_str(),
            parts[1].c_str());
        #endif
        ...
        #if DEBUG_DATE_VALIDATE
        MessageInterface::ShowMessage
            ("DateUtil::IsValidGregorianExtra() dateParts=%s, %s, %s\n", dateParts[0].c_str(),
            dateParts[1].c_str(), dateParts[2].c_str());
        MessageInterface::ShowMessage
            ("DateUtil::IsValidGregorianExtra() timeParts=%s, %s, %s\n", timeParts[0].c_str(),
        ...
        #if DEBUG_DATE_VALIDATE
        MessageInterface::ShowMessage("DateUtil::IsValidGregorianExtra() year=%d, month=%d, day=%d, hour=%d, "
        ...
        #if DEBUG_DATE_VALIDATE
        MessageInterface::ShowMessage("DateUtil::IsValidGregorianExtra() parts=%s, %s\n", parts[0].c_str(),
            parts[1].c_str());
        #endif
        ...
        GmatStringUtil::ToReal( parts[0], &tmpDate );
        UnpackDate(tmpDate, year, month, day);

        Real  tmpTime;
        GmatStringUtil::ToReal( parts[1], &tmpTime );
        UnpackTime(tmpTime, hour, min, sec);
        ...
        MessageInterface::ShowMessage("Warning:  Invalid Gregorian format: %d\n", format);
        return false;
    }
}
...
// ajfg
/**
 * Convert between the two Gregorian Format
 *
 * Valid format    1 = "01 Jan 2000 11:59:28.000"
 *                 2 = "2000-01-01T11:59:28.000"
 *                 3 = "YYYYMMDD.HHMMSSmmm"
 *
 * @param  inStr  input gregorian string
 * @param  inFormat input gregorian string format
 * @param  outStr  output gregorian string
 * @param  outFormat output gregorian string format
 *
 * @return true if time is in valid Gregorian format; otherwise, false
 */
bool DateUtil::ConvertGregorianFormat(const std::string &inStr, const Integer inFormat, 
    std::string &outStr, const Integer outFormat)
{
    if (inFormat == 1) {
       if (outFormat == 1) {
           outStr = inStr;
           return true;
            ...
            if (outFormat == 2) {
                // Convert from 1 = "01 Jan 2000 11:59:28.000"   
                //         to   2 = "2000-01-01T11:59:28.000"

                StringArray parts = GmatStringUtil::SeparateBy(inStr, " ");
                if (parts.size() != 4)
                    return false;

                #if DEBUG_DATE_VALIDATE
                MessageInterface::ShowMessage("DateUtil::ConvertGregorianFormat() parts=%s, %s, %s, %s\n", parts[0].c_str(),
                    parts[1].c_str(), parts[2].c_str(), parts[3].c_str());
                #endif

                outStr = parts[2] + "-" + 
                        GmatStringUtil::ToString( GmatTimeUtil::GetMonth(parts[1]), 2 ) + "-" +
                        parts[0] + "T" +
                        parts[3];
                ...
                // Get the month
                Integer month = GmatTimeUtil::GetMonth(parts[1]);
                ...
                // YYYYMMDD.HHMMSSmmm
                outStr = parts[2] + GmatStringUtil::ToString(month, 2) + parts[0] + "." +
                        timeParts[0] + timeParts[1];

                // Remove '.' from seconds
                StringArray secondsParts = GmatStringUtil::SeparateDots(timeParts[2]);
                if (secondsParts.size() == 2) {
                    // It does include milliseconds
                    outStr += secondsParts[0] + secondsParts[1];
                } else {
                    outStr += secondsParts[0];
                }
    ...
    if (inFormat == 2) {
        if (outFormat == 2) {
           ...
           return true;
            ...
            if (outFormat == 1) {
                // Convert from 2 = "2000-01-01T11:59:28.000"   
                //         to   1 = "01 Jan 2000 11:59:28.000"
                StringArray parts = GmatStringUtil::SeparateBy(inStr, "T");
                if (parts.size() != 2)
                    return false;

                #if DEBUG_DATE_VALIDATE
                MessageInterface::ShowMessage("DateUtil::ConvertGregorianFormat() parts=%s, %s\n", parts[0].c_str(),
                    parts[1].c_str());
                #endif
                ...
                #if DEBUG_DATE_VALIDATE
                MessageInterface::ShowMessage("DateUtil::ConvertGregorianFormat() dateParts=%s, %s, %s\n", dateParts[0].c_str(),
                    dateParts[1].c_str(), dateParts[2].c_str());
                #endif

                Integer tmpMonth;

                GmatStringUtil::ToInteger(dateParts[1], &tmpMonth);
                outStr = dateParts[2] + " " + 
                        GmatTimeUtil::GetMonthName(tmpMonth) + " " +
                        dateParts[0] + " " +
                        parts[1];
                ...
                // YYYYMMDD.HHMMSSmmm
                outStr = dateParts[0] + dateParts[1] + dateParts[2] +
                        timeParts[0] + timeParts[1];
    ...
    if (inFormat == 3) {
       if (outFormat == 3) {
           ...
           return true;
            ...
            if (outFormat == 1) {
                // Convert from 3 = "YYYYMMDD.HHMMSSmmm"   
                //         to   1 = "01 Jan 2000 11:59:28.000"
                
                StringArray parts = GmatStringUtil::SeparateDots(inStr);
                if (parts.size() == 2) 
                    return false;

                Integer month;
                std::string  tmpYear, tmpMonth, tmpDay;

                tmpYear = parts[0].substr(0, 4);

                tmpMonth = parts[0].substr(4, 2);
                GmatStringUtil::ToInteger(tmpMonth, &month);

                tmpMonth = GmatTimeUtil::GetMonthName(month);

                tmpDay = parts[0].substr(6, 2);

                std::string tmpHour, tmpMin, tmpSec, tmpMilli;

                tmpHour   = parts[1].substr(0, 2);
                tmpMin    = parts[1].substr(2, 2);
                tmpSec    = parts[1].substr(4, 2);
                tmpMilli  = parts[1].substr(6);

                // 1 = "01 Jan 2000 11:59:28.000"
                outStr = tmpDay + " " + tmpMonth + " " + tmpYear + " " +
                        tmpHour + ":" + tmpMin + ":" + tmpSec;

                if (tmpMilli.empty() == false) {
                    outStr += ".";
                    outStr += tmpMilli;
                }
                ...
                // 2 = "2000-01-01T11:59:28.000"
                outStr = tmpYear + "-" + tmpMonth + "-" + tmpDay + "T" +
                        tmpHour + ":" + tmpMin + ":" + tmpSec;

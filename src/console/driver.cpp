...
// ajfg
/**
 * Run the GMAT in server mode
 * It will open a socket connection and listen for incoming connectiong
 * 
 */
void RunServer(const uint inPort)
{
   SocketServer   server;

   // It does run in the localhost
   server.SetPort(inPort);
...
/**
 * Run the GMAT in server mode
 * It will open a NNG socket connection and listen for incoming connectiong
 * Then, it will read a message and process it
 * 
 */
int RunNngServer(const std::string& inConfigFile, const std::string& inInstanceId, const std::string& inPullAddress, 
const std::string& inPubAddress, const std::string& inReqAddress, const int inVerbosityFlag)
{
    NngServer   server(inConfigFile, inInstanceId, inPullAddress, inPubAddress, inReqAddress, inVerbosityFlag, mod);
         ...
         if (!runcomplete)
         {
            ...
            for (int i = 1; i < argc; ++i)
            {
               ...
               if (arg == "--start-server")
               {
                  // ajfg
                  //std::cout << "\nGMAT server currently unavailable to GmatConsole\n ";
                  std::string portArg = argv[i+1];
                  uint  portNumber = std::stoi(portArg);

                  RunServer(portNumber);
                  runcomplete = true;
               }
               if (arg == "--start-nng-server")
               {
                   if (argc < (i+6)) 
                   {
                       MessageInterface::ShowMessage("*** Missing parameters\n");
                   }
                   else 
                   {
                      // ajfg
                      // Command line:
                      //   executable  config_file.json   instance_id   pull_address   pub_address   req_address

                      std::string configFile  = argv[i+1];
                      std::string instanceId  = argv[i+2];
                      std::string pullAddress = argv[i+3];
                      std::string pubAddress  = argv[i+4];
                      std::string reqAddress  = argv[i+5];

                      if (RunNngServer(configFile, instanceId, pullAddress, pubAddress, reqAddress, verbosity) != 0)
                        throw ConsoleAppException("Running NNG Server failed");
                      runcomplete = true;   
...
//      MessageInterface::ShowMessage("EXITing GmatConsole with exit code %d", EXIT_FAILURE);
   ...
   std::cout << "Quitting GmatConsole" << std::endl;

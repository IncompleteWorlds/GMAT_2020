...
SocketServer::SocketServer()
   : error{0}, m_numClients{0}, shutdownserver{false}, 
	Server{0}, 
	m_address{"localhost"}, m_port{3000}
{
	error = 0;
	m_numClients = 0;
...
void SocketServer::RunServer()
{
	 ...
	 char* computername = new char[ m_address.length()+1 ];
     strcpy(computername, m_address.c_str());

     if (isalpha(computername[0]))
     {
    	 //addr_info = gethostbyname(IP_ADDRESS);
		 addr_info = gethostbyname( m_address.c_str() );
#ifdef LINUX_MAC
